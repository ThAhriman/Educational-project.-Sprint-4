...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	spentCalForRun := ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return spentCalForRun
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	meanSpeedInMS := meanSpeed(action, duration) * (1000.0 / (60.0 * 60.0))
	heightInMeter := height / 100.0
	spentCalForWalk := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedInMS, 2.0)/heightInMeter)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return spentCalForWalk
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	spentCalForSwim := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return spentCalForSwim
}
